...
void ScreenStreamer::serveHtml(QTcpSocket *socket) {
  static const char *html =
    ...
    "</div>\n"
    "<script>\n"
    "var pollMs=150, lastSz=0, fpsCounter=0, fpsTimer=0, curFps=0;\n"
    "var c3W=1920, c3H=1080;\n"
    "var touchDown=false;\n"
    "var touchStartTime=0;\n"
    "\n"
    "function updateInfo(sz){\n"
    "  lastSz=sz; fpsCounter++;\n"
    "  var now=Date.now();\n"
    ...
    "  x.responseType='blob';\n"
    "  x.onload=function(){if(this.response){updateInfo(this.response.size)}};\n"
    "  x.send();\n"
