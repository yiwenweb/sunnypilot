## ============================================================================
## carcontroller-tao.py  —— 涛哥版 A 方案纯复刻 (供对照测试, 部署时改名为 carcontroller.py)
## ----------------------------------------------------------------------------
## 与你现有 carcontroller.py 的核心区别 (A 方案 = 100% 复刻涛哥):
##   1. 【无任何 LOCK1~7 / ANTISTALL】—— 极简握手。
##   2. 握手: 见 CS.lkas_prepared 立即 lkas_active=1, softstart 从0爬, 不等 CruiseActivated。
##      (已确认你的 panda byd.h 的 steer_req 只取 bit28 LKAS_Active, 不卡 CruiseActivated,
##       故涛哥"立即出力"在你车上不会被 panda 拦。)
##   3. 退出: 用 lat_safeoff 布尔三态 (接管过 -> 平滑收力到0 -> 松手)。
##   4. 【不发 813/815】—— 涛哥靠摄像头原厂透传, 必须配套 byd-tao.h (只拦814/放行813/815)。
##
## import 路径已适配你的 sunnypilot 2026 工程。
## ⚠️ 改名部署时: 本文件改成 carcontroller.py, 且它 import 的 bydcan 必须是改名后的
##    bydcan.py (即把 bydcan-tao.py 改名为 bydcan.py)。
## ============================================================================
import numpy as np
from opendbc.can import CANPacker
from opendbc.car import Bus, structs
from opendbc.car.lateral import apply_driver_steer_torque_limits  # 2026版此函数在 lateral, 不在 opendbc.car
